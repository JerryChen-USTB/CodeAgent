...
本轮采用 Python `prompt_toolkit` 实现 CodeAgent 自己的终端交互层，目标是保留 CLI 产品属性，同时让 wizard 更像可回看、可修改的问卷式任务面板。
...
运行期间，审批和阶段进度使用同一套 Codex 风格滚动输出。计划/补丁审批仍保持“同意”与“提出修改意见”两个核心选项；命令审批保留必要安全选项。
