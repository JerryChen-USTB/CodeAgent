...
| M8 调试模块 | 失败日志读取、复现、失败测试定位、源码搜索、可疑位置排序、根因分析 | 失败日志、测试报告、源码 | fault_localization、root_cause、debug_report | `FailureAnalyzer`, `Reproducer`, `FaultLocalizer`, `RootCauseAnalyzer` |
| M9 修复模块 | 修复计划、最小 patch、风险检查、审批应用、回归验证、多轮修复 | 调试结果、失败测试、源码 | repair.patch、after_test.log、repair_report | `RepairPlanner`, `RepairPatchGenerator`, `PatchRiskChecker`, `RepairVerifier` |
...
| M11 Benchmark 模块 | 读取 benchmark.yaml，批量运行案例，统计成功率和失败原因 | benchmark.yaml | benchmark_result.json/md | `BenchmarkRunner`, `CaseLoader`, `MetricAggregator` |
...
关键设计：调试阶段可能无法自动复现；若无测试命令，则进入基于日志和源码的静态分析路径，并在报告中降低置信度。
...
关键设计：修复失败后不是直接结束，而是在最大轮数内返回调试/修复闭环；达到上限后输出失败报告和下一步建议。
...
| Debugger | 测试日志、失败报告、源码 | fault_localization、root_cause | 读取、搜索、可选 shell 复现 |
| Repairer | root_cause、repair_plan、源码 | repair.patch | 读取、搜索、patch 生成 |
