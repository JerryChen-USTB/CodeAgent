...
from codeagent.benchmark.schemas import CaseExecutionContext
...
class _FakePlanGenerationService:
    def create_testing_request(self, context) -> TestingRequest:
        ...
        if (
            "oracle_tests" not in configured
            and "evaluation" not in configured
            and "private_tests" not in configured
        ...
        return TestingRequest(
            ...
            plan_review=ApprovalDecision(
                ...
                comment="Generated by fake benchmark testing planner.",
                auto=True,
                decided_by="test",
            ),
...
def _use_fake_testing_plan_generation(monkeypatch) -> None:
    monkeypatch.setattr(
...
def test_meeting_room_demo_benchmark_config_selects_single_case() -> None:
    root = Path(__file__).resolve().parents[2]

    loaded = CaseLoader().load(
        root / "benchmark" / "selfbuilt" / "meeting_room_demo_benchmark.yaml"
    )

    assert [case.case_id for case in loaded.enabled_cases] == [
...
def test_case_evaluator_reports_agent_self_test_timeout_with_collected_count(
    tmp_path,
) -> None:
    ...
    stdout_log = logs_dir / "pytest.stdout.log"
    stdout_log.write_text(
        "collecting ... collected 12 items\n"
        "tests/test_app.py::test_one PASSED [  8%]\n",
        encoding="utf-8",
    )
    stderr_log = logs_dir / "pytest.stderr.log"
    stderr_log.write_text("Command timed out after 15 seconds.\n", encoding="utf-8")
        ...
        json.dumps(
            {
                ...
                "timed_out": True,
                "command": "python -m pytest tests -q",
                "log_paths": [str(stdout_log), str(stderr_log)],
            }
        ),
        encoding="utf-8",
    )
    ...
    assert evaluation.agent_test_total == 12
    assert evaluation.agent_test_success is False
...
def test_prepare_case_workspace_preserves_nested_hidden_paths_in_copied_case(
    ...
    assert context.oracle_timeout_seconds == 10
    assert (
        context.task_config.test_command.timeout_seconds
